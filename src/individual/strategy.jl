...
notation.
"""
function plot_strategy(strat::IndividualStrategy, static_s::StaticIndividualState)
    gr()
    ...
    plot!(plt1, 1:length(strat.scores), map(i -> strat.hps[i][1], 1:length(strat.scores)) ./
        static_s.teams[1].stats.hitpoints, color = :green, label = "")
    plot!(plt1, 1:length(strat.scores), map(i -> strat.hps[i][2], 1:length(strat.scores)) ./
        static_s.teams[2].stats.hitpoints, color = :purple, label = "")
    ...
    for i = 1:length(strat.scores), j = Int8(1):Int8(2)
        ...
        elseif strat.decisions[i][j] == 0x05 || strat.decisions[i][j] == 0x06
            color = colors[static_s.teams[j].chargedMoves[strat.decisions[i][j] - 0x04].moveType]
            scatter!(plt2, [i], [-j], markershape = :circle, markersize = 10, alpha = 0.5, color = color)
        end
        if i > 1
            if strat.decisions[i][j] != 0x05 && strat.decisions[i][j] != 0x06 &&
