...
Takes in the dynamic state, the static state, and the attacking agent and 
returns the dynamic state after the fast move has occurred, with precisely one 
copy
"""
function evaluate_fast_move(static_state::StaticState, agent::UInt8, 
    ...
    attacker_energy += 
        get_energy(static_state[agent][attacker_active].fast_move)
...
apply_buff(a1::UInt8, d1::UInt8, a2::UInt8, d2::UInt8, move::ChargedMove) = 
    ApplyBuffOutput(
        clamp(iszero(get_buff_target(move)) ? a1 + get_atk(move.buff) : a1, 
            0x00, 0x09),
        clamp(iszero(get_buff_target(move)) ? d1 + get_def(move.buff) : d1, 
            0x00, 0x09), 
        clamp(!iszero(get_buff_target(move)) ? a2 + get_atk(move.buff) : a2, 
            0x00, 0x09),
        clamp(!iszero(get_buff_target(move)) ? d2 + get_def(move.buff) : d2, 
        0x00, 0x09)
    )
...
"""
    evaluate_charged_move(state, static_state, cmp, move_id, charge, shielding, 
        buffs_applied)
...
"""
function evaluate_charged_move(static_state::StaticState, cmp::UInt8, 
    move_id::UInt8, shielding::Bool, active_1::UInt8, active_2::UInt8, 
    a1::UInt8, d1::UInt8, a2::UInt8, d2::UInt8, attacker_energy::UInt8, 
    defender_hp::UInt16, shields::UInt8, fm_dmg_1::UInt16, fm_dmg_2::UInt16)
    ...
    if buff_chance == 1.0
        ...
        fm_dmg_1 = calculate_damage(static_state[0x01][active_1].stats.attack, 
            ...
            (static_state[0x02][active_2].primary_type, 
                static_state[0x02][active_2].secondary_type), 
        ...
        fm_dmg_2 = calculate_damage(static_state[0x02][active_2].stats.attack, 
            ...
            (static_state[0x01][active_1].primary_type, 
                static_state[0x01][active_1].secondary_type), 
    ...
    return EvaluateChargedMovesOutput(chance, a1, d1, a2, d2, fm_dmg_1, 
        fm_dmg_2, attacker_energy, defender_hp, shields, cmp)
end
...
Takes in the dynamic state, the switching agent, which team member they switch 
to, and the time in the switch (only applies in switches after a faint) and 
returns the dynamic state after the switch has occurred, with precisely one copy
"""
function evaluate_switch(static_state::StaticState, agent::UInt8, 
    to_switch::UInt8, time::UInt8, active_1::UInt8, active_2::UInt8, 
    ...
    return EvaluateSwitchOutput(agent == 0x01 ? active_1 : active_2, 
        switch_cooldown_1, switch_cooldown_2, fm_dmg_1, fm_dmg_2)
end
...
Given the state and the static state (here just for starting hp values), 
compute the PvPoke-like score that would occur if the first agent stopped 
attacking altogether. This is currently only used in computing the final score, 
but it could be used as strict bounds for α/β pruning, for example.
"""
...
Given the state and the static state (here just for starting hp values), 
compute the PvPoke-like score that would occur if the second agent stopped 
attacking altogether. This is currently only used in computing the final score, 
but it could be used as strict bounds for α/β pruning, for example.
"""
...
Given the state and the static state (here just for starting hp values), 
compute the PvPoke-like score for the battle. Note that this can also be 
computed for battles in progress, and thus differs from PvPoke's use cases
"""
