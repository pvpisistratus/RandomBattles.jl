...
function plot_strategy(strat::Strategy, static_s::StaticState)
    ...
    plot!(plt1, 1:length(strat.scores), map(i -> strat.hps[i][1][1]), color = :green, label = "")
    plot!(plt1, 1:length(strat.scores), map(i -> strat.hps[i][1][2]), color = :green, label = "")
    plot!(plt1, 1:length(strat.scores), map(i -> strat.hps[i][1][3]), color = :green, label = "")
    plot!(plt1, 1:length(strat.scores), map(i -> strat.hps[i][2][1]), color = :purple, label = "")
    plot!(plt1, 1:length(strat.scores), map(i -> strat.hps[i][2][2]), color = :purple, label = "")
    plot!(plt1, 1:length(strat.scores), map(i -> strat.hps[i][2][3]), color = :purple, label = "")
