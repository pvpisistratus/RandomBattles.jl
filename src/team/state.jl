...
StaticState(teams::Array{String}; league = "great", cup = "open") =
    StaticState([
        StaticTeam(teams[1:3], league = league, cup = cup,
            opponents = @SVector [i for i in StaticPokemon.($teams[4:6], league = league, cup = cup)]),
        StaticTeam(teams[4:6], league = league, cup = cup,
            opponents = @SVector [i for i in StaticPokemon.($teams[1:3], league = league, cup = cup)])
    ])
