...
function play_turn(state::DynamicState, static_state::StaticState, decision::Tuple{Int64,Int64})
    ...
    if @inbounds next_state.fastMovesPending[1] == Int8(0)
        next_state = evaluate_fast_moves(next_state, static_state, Int8(1))
    end
    if @inbounds next_state.fastMovesPending[2] == Int8(0)
        next_state = evaluate_fast_moves(next_state, static_state, Int8(2))
    end

    next_state = step_timers(next_state,
        @propogate_inbounds 2 < decision[1] < 5 ? static_state.teams[1].mons[next_state.teams[1].active].fastMove.cooldown : Int8(0),
        @propogate_inbounds 2 < decision[2] < 5 ? static_state.teams[2].mons[next_state.teams[2].active].fastMove.cooldown : Int8(0))
    ...
    if @inbounds cmp[1] != Int8(0)
        @propogate_inbounds next_state = evaluate_charged_moves(next_state, static_state, cmp[1],
    ...
    if @inbounds cmp[2] != Int8(0)
        @propogate_inbounds next_state = evaluate_charged_moves(next_state, static_state, cmp[2],
