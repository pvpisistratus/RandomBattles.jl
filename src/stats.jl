...
Stores the attack, defense, and hitpoints values (factoring in base and IV
stats) of a Pokemon in integers. This means that attack and defense are 100
times larger than their real value.
"""
struct Stats
    attack::UInt16
    defense::UInt16
...
"""
    StatBuffs(val)
Because the stat buffs can only be between -4 and 4 inclusive, they are
collectively stored as one 8-bit integer. Use of this constructor is not
recommended. Also, use get_atk and get_def functions for extracting individual
values.
"""
struct StatBuffs
    val::UInt8
...
function StatBuffs(atk::Int8, def::Int8)
    StatBuffs((clamp(atk, Int8(-4), Int8(4)) + Int8(8)) +
        (clamp(def, Int8(-4), Int8(4)) + Int16(8))<<Int16(4))
end
...
"""
Base.:+(x::StatBuffs, y::StatBuffs) = StatBuffs(get_atk(x) + get_atk(y),
    get_def(x) + get_def(y))
