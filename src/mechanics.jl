...
function evaluate_fast_moves(state::DynamicState, static_state::StaticState, agent::Int64)
    next_state = state
...
function evaluate_charged_moves(state::State, static_state::StaticState, dec::Decision)
    ...
    if cmp > Int8(0)
        ...
        if next_state.fastMovesPending[other_agent] != Int8(-1)
            next_state = evaluate_fast_moves(next_state, static_state, cmp == Int8(2) ? 1 : 2)
        end
