...
function step_timers(state::DynamicState)
    return DynamicState([DynamicTeam(state.teams[1].mons, state.teams[1].buffs, max(Int8(0), state.teams[1].switchCooldown - Int8(1)), state.teams[1].shields, state.teams[1].active),
        ...
        [max(Int8(-1), state.fastMovesPending[1] - Int8(1)), max(Int8(-1), state.fastMovesPending[2] - Int8(1))],
    )
end
